    ...
    router.get('/stats', async (_req, res) => {
        try {
            ...
            try {
                ;
                [projectCount, totalSupply] = await Promise.all([
                    publicClient.readContract({ address: addrs.registry, abi: abis_js_1.REGISTRY_ABI, functionName: 'projectCount' }),
                    publicClient.readContract({ address: addrs.token, abi: abis_js_1.TOKEN_ABI, functionName: 'totalSupply' }),
                ]);
            }
