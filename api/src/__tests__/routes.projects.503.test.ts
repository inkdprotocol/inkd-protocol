/**
 * @inkd/api — routes/projects.ts — requireRegistry() 503 branch
 *
 ...
 * (empty registry address) without interfering with routes.projects.test.ts.
 */
...
describe('GET /v1/projects — requireRegistry() ServiceUnavailable (503)', () => {
...
describe('GET /v1/projects/:id — requireRegistry() ServiceUnavailable (503)', () => {
  it('returns 503 when registry address is not configured', async () => {
    const app = await makeApp()
...
describe('GET /v1/projects/:id/versions — requireRegistry() ServiceUnavailable (503)', () => {
  it('returns 503 when registry address is not configured', async () => {
    const app = await makeApp()
...
describe('POST /v1/projects/:id/versions — requireRegistry() ServiceUnavailable (503)', () => {
  it('returns 503 when registry address is not configured', async () => {
    const app = await makeApp()
    const res = await request(app)
      .post('/v1/projects/1/versions')
      .send({ tag: 'v1.0.0', contentHash: 'abc123', metadataHash: 'def456' })
    expect(res.status).toBe(503)
    expect(res.body.error.code).toBe('SERVICE_UNAVAILABLE')
  })
})
